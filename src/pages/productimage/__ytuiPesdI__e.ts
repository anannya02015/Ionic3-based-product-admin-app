...
let makeFirstDefault:boolean = false
  for(var i in this.processedImages) {   
    ...
    imageRec.prodid=this.productid     
 ...
 if(this._damrusetDUMDUM__){
  this.restapiService.___damrusetDUMDUMNO__(this.productid).then((result) => {
    console.log(result);    
  }, (err) => {
    loader.dismiss();
 ...
 setTimeout(() => {               
  ...
  setTimeout(() => { 
    ...
    if(this.mode=="edit"){
           ...
           // this.storage.set("'"+this.productid+"'",JSON.stringify(this.product))
